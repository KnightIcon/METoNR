...
class GRUModel(BaseModel):
    ...
    def _build_userencoder(self, news_encoder):
        ...
        his_emb_dense = news_encoder(his_input_emb)
        user_present = layers.GRU(hparams.article_emb_size)(his_emb_dense)
        user_present_dence = layers.Dense(hparams.dense_dim,
                      kernel_regularizer=l2(0.001),
                      kernel_initializer='glorot_normal',
                      activation=hparams.dense_activation)(user_present)
        model = keras.Model(his_input_emb, user_present_dence, name="user_encoder")
        return model
